...
#w = tf.get_variable(name='weights', shape=(56*56, 10), initializer=tf.random_normal_initializer(0, 0.01))
L = 40

w = tf.get_variable("w", shape=(56*56*4,L), initializer=tf.random_normal_initializer(0,0.01))
b = tf.get_variable("b", shape=(1,L), initializer=tf.zeros_initializer())

w2 = tf.get_variable("w2", shape=(L,37), initializer=tf.random_normal_initializer(0,0.01))
b2 = tf.get_variable("b2", shape=(1,37), initializer=tf.zeros_initializer())
...
with tf.Session() as sess:
    ...
    total_cmat = np.zeros((37,37),dtype="int32")
    try:
        while True:
